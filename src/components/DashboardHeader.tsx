...
interface DashboardHeaderProps {
  activeSection: 'all' | 'top' | 'middle' | 'bottom';
      ...
      <div className="max-w-xl mx-auto px-4 py-3">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#666666] font-medium leading-none mt-0.5">
                Singapore Condo Sales Manager
              </p>
            </div>
          </div>

          {/* 8pm Closing Status Badge with Navy Styling */}
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#1B2A4A]/10 text-[#1B2A4A] border border-[#1B2A4A]/20 text-xs font-semibold">
            <span className="w-2 h-2 rounded-full bg-[#1B2A4A] animate-pulse"></span>
            <span>8:00 PM Closing</span>
          </div>
        </div>

        {/* Section Quick Jump Navigation */}
        <div className="flex items-center gap-1.5 mt-3 pt-2 border-t border-[#F0F0F0]">
          <button
            id="nav-top-section"
          ...
          <button
            ...
            onClick={() => onScrollToSection('middle-salesperson-ranking-section')}
            className="flex-1 text-center py-1.5 px-2 rounded-lg text-xs font-semibold bg-[#F5F5F5] hover:bg-[#EBEBEB] text-[#1B2A4A] transition-colors shrink-0"
          >
            2. Team Ranking
          </button>
